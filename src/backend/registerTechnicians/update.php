...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare('UPDATE technicians SET name = :name, number = :number WHERE id = :id');
        $stmt->bindValue(':name', $name, PDO::PARAM_STR);
        $stmt->bindValue(':number', $number, PDO::PARAM_STR);
        $stmt->bindValue(':id', $id, PDO::PARAM_INT);
        $stmt->execute();
        if ($stmt->rowCount() > 0) {
            $_SESSION['success_message'] = "Técnico atualizado com sucesso.";
        } else {
            $_SESSION['error_message'] = "Nenhuma alteração feita ou Técnico não encontrado.";
        }
        header('Location: /davidServices/pages/registerTechnicians');
        exit;
    } catch (PDOException $e) {
        $_SESSION['error_message'] = "Erro ao Atualizar o Técnico: " . $e->getMessage();
        header('Location: /davidServices/pages/registerTechnicians');
        exit;
    }
}
